...
function getTranslatedMessage(messageName, substitutions) {
  ...
  if (!translatedMessage) {
    throw new Error(
      `no translation string for "${messageName}" could be found`,
    );
  }
...
function innerTranslateTextNodes(parent, translatedMessage, subsContainer) {
  const splitTranslatedMessage = translatedMessage.split(
    UNIQUE_REPLACEMENT_SPLIT,
  );

  console.log(
    "Replacing text nodes for",
    parent,
    ", message:",
    translatedMessage,
    "detected elements:",
  ...
  if (splitTranslatedMessage.length <= subsContainer.substitutions.length) {
    console.warn(
      ...
      subsContainer.substitutions.length,
      "substitutions. Possibly you did not include all substitutions in your translation?",
...
function getSubitems(elem) {
  ...
  const textOnlyChilds = childs.filter(
    (node) => node.nodeType === Node.TEXT_NODE,
  );

  // create list of substitutions, i.e. $1, $2, §3 etc.
  const substitutions = htmlOnlyChilds.map(
    (elem, num) =>
      `${UNIQUE_REPLACEMENT_SPLIT}${UNIQUE_REPLACEMENT_ID}${num + 1}${UNIQUE_REPLACEMENT_SPLIT}`,
  );
...
function replaceI18n(elem, tag) {
  ...
  for (const [dataAttribute, dataValue] of Object.entries(elem.dataset)) {
    ...
    const replaceAttribute = convertDatasetToAttribute(
      dataAttribute.slice(I18N_DATASET_INT),
    );
